...
The parent turns it on and answers six chips. The child never sees that form. The first sentence uses the door from intake. The camera is a valid start.
...
```
Child:  foto av läxan
Utter: Alma. Redstone and fractions are the same thing. What's fog on the picture?
```

Never the answer first. Pace that fits. Stops when it gets hard. Visible to the parent. Not a child dashboard. Not a Shorts loop.
...
[http://127.0.0.1:8080/start.html](http://127.0.0.1:8080/start.html) — name, key, consent.
Then six chips + skip. Then the parent preview. Then chat: first sentence + camera.
