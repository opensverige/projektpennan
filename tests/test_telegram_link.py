...
def test_empty_state_cannot_start(monkeypatch, tmp_path):
    monkeypatch.setenv("GNISTA_TELEGRAM_STATE", str(tmp_path / "telegram.json"))
    monkeypatch.setenv("GNISTA_TELEGRAM_TOKEN_FILE", str(tmp_path / "token.txt"))
...
def test_verify_binds_invite_without_leaking_token(monkeypatch, tmp_path):
    ...
    def fake_me(_token):
        return {"username": "AlmaGnista_bot", "id": 1}

    result = verify_and_bind("123456:AA-real-looking-token-xx", fetcher=fake_me)
    assert result["ok"] is True
    assert result["username"] == "AlmaGnista_bot"
    assert result["invite"].startswith("https://t.me/AlmaGnista_bot?start=")
...
def test_start_token_claims_only_matching_chat(monkeypatch, tmp_path):
    monkeypatch.setenv("GNISTA_TELEGRAM_STATE", str(tmp_path / "telegram.json"))
    monkeypatch.setenv("GNISTA_TELEGRAM_TOKEN_FILE", str(tmp_path / "token.txt"))
    bind_token("123456:AA-real-looking-token-xx", "AlmaGnista_bot")
